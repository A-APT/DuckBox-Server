...
class GroupControllerTest {
    ...
    @Test
    fun is_removeGroup_throws_no_authToken_header() {
        // arrange
        val httpEntity = HttpEntity(ObjectId().toString(), HttpHeaders())

        // act, assert
        restTemplate
            .exchange("${baseAddress}/api/v1/group", HttpMethod.DELETE, httpEntity, Unit::class.java)
            .apply {
                Assertions.assertThat(statusCode).isEqualTo(HttpStatus.FORBIDDEN)
            }
    }
    ...
    fun is_removeGroup_throws_on_invalid_groupId() {
        ...
        // act, assert
        restTemplate
            .exchange("${baseAddress}/api/v1/group", HttpMethod.DELETE, httpEntity, NotFoundException::class.java)
            .apply {
                Assertions.assertThat(statusCode).isEqualTo(HttpStatus.NOT_FOUND)
            }
    }
    ...
    @Test
    fun is_removeGroup_throws_on_not_group_owner() {
        // arrange
        val token: String = registerAndLogin()
        registerMockUser2()
        val httpHeaders = HttpHeaders().apply {
        ...
        // act, assert
        restTemplate
            .exchange("${baseAddress}/api/v1/group", HttpMethod.DELETE, httpEntity, ForbiddenException::class.java)
            .apply {
                Assertions.assertThat(statusCode).isEqualTo(HttpStatus.FORBIDDEN)
            }
    }
